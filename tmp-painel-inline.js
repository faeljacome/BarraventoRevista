        ...
        (() => {
          ...
          function renderMemberSelectOptions() {
            const options = ['<option value="">Selecione</option>'].concat(
              memberDirectory.map((item) => '<option value="' + escapeHtml(item.email || "") + '">' + escapeHtml(item.name || item.email || "") + '</option>')
            );
            ...
            if (createAuthorMembers) {
              createAuthorMembers.innerHTML = options.slice(1).join("");
            }
            if (editAuthorMembers) {
              editAuthorMembers.innerHTML = options.slice(1).join("");
            }
          ...
          function applyMemberState(nextMember) {
            ...
            if (authenticated) {
              ...
              if (member.role === "admin") {
                setStatus(memberStatus, "success", "Conta com acesso de <strong>Conselho Editorial</strong>. As abas de aprovacoes, logs e cadastro de membros ficam liberadas.");
              } else {
                setStatus(memberStatus, "success", "Conta com acesso de <strong>Revisor</strong>. As abas <strong>Aprovacoes</strong>, <strong>Logs</strong> e <strong>Cadastrar membro</strong> aparecem somente para o Conselho Editorial. O <strong>Editar perfil</strong> foi aberto automaticamente.");
              }
              if (shouldInitializePanel) {
